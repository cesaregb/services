...
public class ClientPaymentInfoDAO implements IClientPaymentInfoDAO{
  ...
  private EntityManager em;
	...
	// This is the other way to generate a custom Query in the dao, not thru the repository but accessing directly the EM.
	public List<ClientPaymentInfo> findByIdClient(Integer idClient) {
		TypedQuery<ClientPaymentInfo> query = em.createQuery("SELECT cpi FROM ClientPaymentInfo cpi WHERE cpi.client.idClient=?1", ClientPaymentInfo.class);
