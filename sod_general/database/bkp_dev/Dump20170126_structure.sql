...
CREATE DEFINER = CURRENT_USER TRIGGER `Clients_BEFORE_UPDATE` BEFORE UPDATE ON `Clients` FOR EACH ROW
BEGIN
SET NEW.updated = NOW();
END$$
DELIMITER ;
...
INSERT INTO `Clients` VALUES (1,1,'email@domain.com','notused','Brad','Pit','twitter',NULL ,NULL,'123',NULL,NULL,0,'333345666',NULL,NULL);
...
CREATE DEFINER = CURRENT_USER TRIGGER `Orders_BEFORE_INSERT` BEFORE INSERT ON `Orders` FOR EACH ROW
BEGIN
SET NEW.created = NOW();
END$$

DROP TRIGGER IF EXISTS `Orders_BEFORE_UPDATE` $$
CREATE DEFINER = CURRENT_USER TRIGGER `Orders_BEFORE_UPDATE` BEFORE UPDATE ON `Orders` FOR EACH ROW
BEGIN
SET NEW.updated = NOW();
END$$
DELIMITER ;
